...
$sql = "SELECT nrt.id, nrt.short_name, nrt.name, nrt.description, nrt.is_active FROM NOM_ROOM_TYPES nrt ".order($selectedCol).$direction;
$result = $conn->query($sql);
$nomRoomTypesList = array();
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
	...
	array_push ($nomRoomTypesList , $nomRoomType);
}
?>
<html>
	<head>
		...
		<title>Списък - Типове стаи</title>
	</head>
	<body>
		<form name="nom_room_types_form" id='nom_room_types_form'>
			<input type="hidden" name="sortColName" id="sortColName"/>
			<script>
				function setOrder(name){
					$('#sortColName').val(name);
					document.getElementById("nom_room_types_form").submit();
				}
			</script>
			<table class="dataset">
				<thead>
					<tr>
						<th><a href="NomRoomTypesEdit.php"><img src="../style/plus.png" title="Нов запис"  style="border: 0" height="16" width="16"></a></th>
				...
				<tbody>
					<?php foreach($nomRoomTypesList as $val): ?>
						<tr>
							<td><a href="NomRoomTypesEdit.php?roomTypeId=<?= $val->getId() ?>"><img src="../style/edit.png" title="Редактиране"  style="border: 0"></a>
							</td>
							<td><?= $val->getId() ?> </td>

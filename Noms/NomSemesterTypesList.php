<?php
include '..'.DIRECTORY_SEPARATOR.'models'.DIRECTORY_SEPARATOR.'Noms'.DIRECTORY_SEPARATOR.'NomSemesterTypes.php';
...
$sql = "SELECT nst.id, nst.short_name, nst.name, nst.description, nst.is_active FROM NOM_SEMESTER_TYPES nst ".order($selectedCol).$direction;
$result = $conn->query($sql);
$nomSemesterTypesList = array();
while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
	$nomSemesterType = new NomSemesterTypes();
	$nomSemesterType->setId($row['id']);
	$nomSemesterType->setShortName($row['short_name']);
	$nomSemesterType->setName($row['name']);
	$nomSemesterType->setDescription($row['description']);
	$nomSemesterType->setIsActive($row['is_active']);
	array_push ($nomSemesterTypesList , $nomSemesterType);
}
?>
<html>
	<head>
		...
		<title>Списък - Типове семестри</title>
	</head>
	<body>
		<form name="nom_semester_types_form" id='nom_semester_types_form'>
			<input type="hidden" name="sortColName" id="sortColName"/>
			<script>
				function setOrder(name){
					$('#sortColName').val(name);
					document.getElementById("nom_semester_types_form").submit();
				}
			</script>
			<table class="dataset">
				<thead>
					<tr>
						<th><a href="NomSemesterTypesEdit.php"><img src="../style/plus.png" title="Нов запис"  style="border: 0" height="16" width="16"></a></th>
				...
				<tbody>
					<?php foreach($nomSemesterTypesList as $val): ?>
						<tr>
							<td><a href="NomSemesterTypesEdit.php?semesterTypeId=<?= $val->getId() ?>"><img src="../style/edit.png" title="Редактиране"  style="border: 0"></a>
							</td>
							<td><?= $val->getId() ?> </td>

...
function save($conn, $post) {
    if ($post['id'] != null && !empty($post['id'])) {
        $stmt = $conn->prepare("UPDATE Rooms SET type_id = ?, num = ?, place = ?,work_stations = ?, description = ?, is_active = ? WHERE ID = ?");
        $stmt->execute(array($post['typeId'], $post['num'], $post['place'], $post['description'],$post['work_stations'], $post['isActive'], $post['id']));
    } else {
        $stmt = $conn->prepare("INSERT INTO Rooms (type_id, num, place, description,  work_stations, is_active) VALUES (?,?,?,?,?)");
        $stmt->execute(array($post['typeId'], $post['num'], $post['place'], $post['description'],$post['work_stations'], $post['isActive']));
    }
...
if (isset($studyProgramId)) {
    $sql = "SELECT r.id, r.num, r.type_id, r.place, r.work_stations, r.description, r.is_active, nrt.name type_name " .
            "FROM Rooms r JOIN nom_Room_types  nrt ON  nrt.id = r.type_id where r.id = ?";
    $stmt = $conn->prepare($sql);
    if ($stmt->execute(array($studyProgramId))) {
        while ($row = $stmt->fetch()) {
            $currRoom = new Rooms();
            $currRoom->setId($row['id']);
            $currRoom->setNum($row['num']);
            $currRoom->setPlace($row['place']);
            ...
            $currRoom->setWorkStations($row['work_stations']);
            $currRoom->setDescription($row['description']);
            $currRoom->setIsActive($row['is_active']);
        }
    }
}

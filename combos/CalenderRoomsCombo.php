...
$sql = "SELECT r.id, r.num, r.place, r.type_id as room_type_id, r.work_stations, r.description, r.is_active "
        . "FROM Rooms r "
        . "WHERE r.is_active = 'Y' and r.type_id = coalesce(?, r.type_id) order by r.num";
$stmt = $conn->prepare($sql);

$rooms = array();
if ($stmt->execute(array(empty($typeId) ? null :$typeId))) {
...
<select name="room" id="room" required="Y">
    ...
    <?php foreach ($rooms as $val): ?>
        <option value="<?= $val->getId() ?>"><?= $val->getNum() ?>(<?= $val->getPlace() ?>)</option>
    <?php endforeach; ?>
</select>